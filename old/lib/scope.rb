class Scope
  ...
  def addArg(name, line, column)
    ...
    error("Cannot redefine #{name}", line, column) if @vars.include?(name)
    @args[name] = :undefined
  end

  def defineArg(name, value, line, column)
    error("Cannot redefine #{name}", line, column) if @vars.include?(name) || (@args.include?(name) && @args[name] != :undefined)
    @args[name] = value
  end
  ...
  def declared?(name)
    @vars.include?(name) || @args.include?(name) || (!@parent.nil? && @parent.declared?(name))
  end
end
